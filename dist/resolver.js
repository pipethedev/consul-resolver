...
class ConsulResolver {
  ...
  async selectOptimalService(
    service,
    algorithm = types_1.SelectionAlgorithm.RoundRobin,
  ) {
    try {
    ...
    const healthyServices = services
      ...
      .map((service) => {
        const serviceMetrics =
          metrics.get(service.Service.ID) || types_1.DEFAULT_METRICS;
        return {
          service,
  ...
  async getServicesMetrics(services) {
    ...
    try {
      ...
      services.forEach((service, index) => {
        ...
        if (error) {
          console.warn(
            `Error getting metrics for service ${serviceId}:`,
            error,
          );
          metricsMap.set(serviceId, { ...types_1.DEFAULT_METRICS });
          return;
        ...
        try {
          ...
          console.warn(
            `Error parsing metrics for service ${serviceId}:`,
            parseError,
          );
          metricsMap.set(serviceId, { ...types_1.DEFAULT_METRICS });
        }
  ...
  async rankServices(services, metrics) {
    return services
      .map((service) => {
        ...
        const totalScore =
          healthScore * types_1.DEFAULT_WEIGHTS.health +
  ...
  async incrementConnections(serviceId) {
    try {
      ...
      await this.redis.set(key, JSON.stringify(metrics), "EX", 24 * 60 * 60);
    } catch (error) {
      console.warn(
